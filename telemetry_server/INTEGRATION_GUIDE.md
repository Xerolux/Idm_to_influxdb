...
Clients should periodically (e.g., every 60 seconds) submit collected metrics to the server.
...
| `data` | List | **Yes** | A list of measurement objects. Allows batching multiple minutes of data in one request. |
...
| Field | Type | Required | Description |
| :--- | :--- | :--- | :--- |
| `timestamp` | Float | **Yes** | Unix timestamp (seconds since epoch), ideally with millisecond precision. |
| `*` | Number/Bool | No | Dynamic metric fields (e.g., `temp_outdoor`, `pressure_high`). Any numeric or boolean field is accepted. |

### Best Practices

1.  **Batching:** Do not send a request for every single second. Buffer data for 1-5 minutes and send a batch. This reduces network overhead and server load.
