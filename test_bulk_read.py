...
def main():
    ...
    logger.info(f"\n{'='*60}")
    logger.info(f"Read blocks optimization: {len(blocks)} blocks for {total_sensors} sensors")
    logger.info(f"{'='*60}")
    ...
    for i, block in enumerate(blocks):
        ...
        logger.info(f"Block {i+1}: Address {start_addr}-{end_addr} ({count} registers, {len(block)} sensors)")
        for sensor in block:
    ...
    if sensors_in_blocks != total_sensors:
        logger.error(f"⚠️  MISSING SENSORS! {total_sensors - sensors_in_blocks} sensors not in any block!")
    ...
    if missing_data:
        ...
        for sensor_name in sorted(missing_data):
            sensor = modbus.sensors.get(sensor_name) or modbus.binary_sensors.get(sensor_name)
            if sensor:
                logger.warning(f"  - {sensor_name} @ address {sensor.address} (read_supported: {sensor.read_supported})")
    else:
