<template>
  <div class="space-y-3 overflow-y-auto">
    ...
    <div
      v-for="(categoryMetrics, category) in filteredMetrics"
      :key="category"
      ...
      <div class="space-y-1">
        ...
        >
          <div class="flex flex-col min-w-0">
            <span class="text-gray-700 font-medium truncate" :title="metric.display">{{
              metric.display
            }}</span>
            <span class="text-[10px] text-gray-400 font-mono truncate">{{ metric.name }}</span>
          </div>
          <span
            class="font-mono font-bold whitespace-nowrap ml-2"
            :class="getValueClass(metric.name, currentValues[metric.name]?.value)"
          >
