  ...
  >
    <div class="space-y-4">
      <div>
        ...
        <InputText v-model="localHeatmap.title" class="w-full" placeholder="Heatmap-Titel" />
      </div>
      ...
      <div>
        ...
        <Select
          ...
          class="w-full"
        />
        <p class="text-xs text-gray-500 mt-1">Anzahl der Zeitabschnitte auf der X-Achse</p>
      </div>
      ...
      <div>
        ...
        <div class="space-y-3">
          <div class="flex items-center gap-2">
            ...
            <ColorPicker v-model="localHeatmap.colorScale.min" format="hex" class="w-10" />
            <InputText v-model="localHeatmap.colorScale.min" class="flex-grow text-sm" />
          </div>
          <div class="flex items-center gap-2">
            <span class="text-sm text-gray-600 w-16">Mitte:</span>
            <ColorPicker v-model="localHeatmap.colorScale.mid" format="hex" class="w-10" />
            <InputText v-model="localHeatmap.colorScale.mid" class="flex-grow text-sm" />
          </div>
          <div class="flex items-center gap-2">
            <span class="text-sm text-gray-600 w-16">Max:</span>
            <ColorPicker v-model="localHeatmap.colorScale.max" format="hex" class="w-10" />
            <InputText v-model="localHeatmap.colorScale.max" class="flex-grow text-sm" />
          </div>
        </div>
        <p class="text-xs text-gray-500 mt-2">Farbverlauf von niedrigen zu hohen Werten</p>
      </div>
      ...
      <div>
        ...
        <p class="text-xs text-gray-500 mt-1">Leer lassen für automatische Bereichserkennung</p>
      </div>
    </div>

    <template #footer>
      <Button @click="visible = false" label="Abbrechen" severity="secondary" text />
      <Button @click="save" label="Speichern" severity="primary" />
    </template>
  </Dialog>
</template>
