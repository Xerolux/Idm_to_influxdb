<template>
  ...
  >
    ...
    <component :is="'style'" v-if="sanitizedCustomCss" :data-dashboard-id="currentDashboardId">
      {{ sanitizedCustomCss }}
    </component>
    ...
    <div class="flex-grow min-h-0 flex flex-col lg:flex-row gap-3 lg:overflow-hidden">
      ...
      <div class="flex-grow overflow-y-auto pb-4 pr-1 relative" @dragover.prevent @drop="onDrop">
        <div
          v-if="isDraggingSensor"
        ...
        <!-- Empty State / Welcome -->
        <div
          v-if="currentCharts.length === 0"
          class="h-full flex flex-col items-center justify-center text-center p-8 text-surface-500"
        >
          <div
            class="bg-surface-0 dark:bg-surface-800 p-8 rounded-xl shadow-sm border border-surface-200 dark:border-surface-700 max-w-md"
          >
            <div
            ...
            <p class="mb-8 text-sm text-surface-600 dark:text-surface-400">
              Starte jetzt, indem du Sensoren aus der linken Leiste hierher ziehst oder eine
              vorgefertigte Vorlage verwendest.
            </p>
            <div class="flex flex-col gap-3">
              <Button
                label="Neuen Chart erstellen"
                icon="pi pi-plus"
                size="large"
    ...
    >
      <div class="space-y-4">
        ...
        <div>
          ...
          <InputText v-model="newChart.title" class="w-full" placeholder="Chart-Titel" />
        </div>
        <div>
      ...
      <template #footer>
        <Button @click="showAddChartDialog = false" label="Abbrechen" severity="secondary" text />
        <Button
          @click="addChart"
          label="Hinzufügen"
          severity="primary"
    ...
    >
      <div class="space-y-4">
        ...
        <div v-if="variables.length === 0" class="text-center py-8 text-gray-500 text-sm">
        ...
        >
          <div class="flex-grow">
            ...
            <div class="text-xs text-gray-500">
              ID: ${{ '{' + variable.id + '}' }} | Typ: {{ variable.type }} |
