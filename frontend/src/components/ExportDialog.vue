              ...
              <span v-if="selectedFormat === 'png'">
                ...
                {{
                  scale === 2
                    ? 'Standardqualität (2x).'
                    : scale > 2
                      ? 'Hohe Qualität (' + scale + 'x) - größere Datei.'
                      : 'Niedrige Qualität - kleine Datei.'
                }}
              </span>
              <span v-else>
                PDF eignet sich für Dokumentation und Druck. Automatisch an A4 (Querformat)
                angepasst.
              </span>
            </p>
            <p v-else>
              ...
              <span v-else-if="selectedFormat === 'excel'">
                Excel-Dateien enthalten mehrere Sheets: Alle Daten, einzelne Metriken und
                Statistiken.
              </span>
...
const getTimeRange = () => {
  ...
  if (timeRange.value === 'custom') {
    start = customStartDate.value
      ? Math.floor(new Date(customStartDate.value).getTime() / 1000)
      : now - 86400
