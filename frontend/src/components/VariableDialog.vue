<template>
  ...
  >
    <div class="space-y-4">
      ...
      <div v-if="localVariable.type === 'query'">
        ...
        <Textarea
          ...
          class="w-full"
        />
        <p class="text-xs text-gray-500 mt-1">Metric query to fetch possible values</p>
      </div>
      ...
      <div class="space-y-2">
        <div class="flex items-center gap-2">
          <Checkbox v-model="localVariable.multi" binary inputId="multi" />
          <label for="multi" class="text-sm text-gray-700"> Mehrfachauswahl erlauben </label>
        </div>
      </div>
    </div>
    ...
    <template #footer>
      ...
      <Button @click="handleDelete" v-if="variable" label="Löschen" severity="danger" text />
      <Button
        @click="handleSave"
        label="Speichern"
        severity="primary"
