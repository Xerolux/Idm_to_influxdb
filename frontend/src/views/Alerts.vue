  ...
  <div class="space-y-6">
          ...
          <div class="flex items-center justify-between">
            <div class="flex-1 min-w-0">
              <div class="flex items-center space-x-3">
                ...
                <span
                  :class="{
                    'bg-green-100 text-green-800': alert.enabled,
                    'bg-gray-100 text-gray-800': !alert.enabled
                  }"
              ...
              <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                <span v-if="alert.type === 'threshold'">
                  Wenn <strong>{{ alert.sensor }}</strong> {{ alert.condition }}
                  <strong>{{ alert.threshold }}</strong>
                </span>
                <span v-else> Regelmäßiger Statusbericht </span>
                <span class="mx-2">•</span>
                <span>Intervall: {{ formatInterval(alert.interval_seconds) }}</span>
              </p>
              <p class="mt-1 text-xs text-gray-400 font-mono truncate">"{{ alert.message }}"</p>
            </div>
    ...
    >
      <form id="alert-form" @submit.prevent="saveAlert" class="space-y-4">
        ...
        <!-- Type -->
        <div>
          <label for="alert-type" class="block text-sm font-medium text-gray-700 dark:text-gray-300"
            >Typ</label
          >
          <select
            id="alert-type"
        ...
        <div
          ...
          class="space-y-4 border-l-4 border-blue-500 pl-4 py-2 bg-gray-50 dark:bg-gray-900"
        >
          <div>
            <label
              for="alert-sensor"
            ...
            <select
              ...
              class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
            >
              <option v-for="s in sensors" :key="s.name" :value="s.name">
                {{ s.name }} ({{ s.unit }})
              </option>
            </select>
          </div>
        ...
        <div>
          ...
          <p class="mt-1 text-xs text-gray-500">
            {{
              form.type === 'status'
                ? 'Wie oft soll der Status gesendet werden (z.B. 86400 für täglich).'
                : 'Wie lange soll nach einem Alarm gewartet werden, bis erneut gesendet wird.'
            }}
          </p>
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            rows="3"
            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
            placeholder="Gib hier deine individuelle Nachricht ein..."
          ></textarea>
          <p class="mt-1 text-xs text-gray-500">
            Du kannst diesen Text bearbeiten. Verfügbare Platzhalter: {value}, {sensor}, {name},
            {time}
          </p>
        </div>
      </form>
...
async function toggleAlert(alert) {
  try {
    ...
    toast.add({
      severity: 'success',
      summary: 'Erfolg',
      detail: `Alarm ${alert.enabled ? 'deaktiviert' : 'aktiviert'}`,
      life: 2000
    })
    await fetchAlerts()
  } catch (e) {
