        ...
        <Card v-for="job in jobs" :key="job.id" class="bg-gray-800 text-white relative">
          <template #title>
            <div class="flex justify-between items-center">
              ...
              <Tag
                :severity="job.enabled ? 'success' : 'warning'"
                :value="job.enabled ? 'Aktiv' : 'Pausiert'"
              />
            </div>
          </template>
          <template #content>
...
const fetchSchedule = async () => {
  ...
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Fehler',
      detail: 'Zeitplan konnte nicht geladen werden',
      life: 3000
    })
  } finally {
    loading.value = false
...
const toggleJob = async (id, currentState) => {
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Fehler',
      detail: 'Umschalten fehlgeschlagen',
      life: 3000
    })
  }
}
...
const runJob = async (id) => {
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Fehler',
      detail: 'Ausführung fehlgeschlagen',
      life: 3000
    })
  }
}
